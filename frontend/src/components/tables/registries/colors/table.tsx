...
import { Color } from "~/types/registries";
import { getColorsTableColumns } from "./columns";
import { useDataTable } from "~/hooks/use-data-table";
import { useColors } from "~/hooks/use-registries";
import { DataTable } from "~/components/ui/data-table";
import { useColorsStore } from "~/store/registries";
...
export function ColorsTable() {
  const { setFilters } = useColorsStore();
  ...
  const color = data?.results ?? [];
  const rowCount = data?.count ?? 0;

  const [rowAction, setRowAction] =
    React.useState<DataTableRowAction<Color> | null>(null);

  const columns = React.useMemo(
    () => getColorsTableColumns({ setRowAction }),
    [],
  );
